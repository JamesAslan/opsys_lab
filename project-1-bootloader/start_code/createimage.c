...
uint8_t count_kernel_sectors(Elf32_Phdr *Phdr)
{
    uint8_t result = ((*Phdr).p_filesz+511 ) / 512;
    return result;
}
...
void write_kernel(FILE *image, FILE *knfile, Elf32_Phdr *Phdr, int kernelsz)
{
    ...
    while(kernelsz > 0)
    {
        fread (middleground,1,512,knfile);
        fwrite (middleground,1,512,image);
        fseek(knfile, 512, SEEK_CUR);
