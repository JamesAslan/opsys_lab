...
void system_call_helper(int fn, int arg1, int arg2, int arg3)
{
    ...
    syscall_handler[fn](arg1, arg2, arg3);
}
...
{
    invoke_syscall(SYSCALL_CLEAR, (int)line1, (int)line2, IGNORE);
}

void sys_ps(int arg)
{
    invoke_syscall(SYSCALL_PROCESS_SHOW, (int)arg, IGNORE, IGNORE);
}
...
void barrier_init(barrier_t *barrier, int arg)
{
    invoke_syscall(SYSCALL_BARRIER_INIT, (int)barrier, (int)arg, IGNORE);
}
...
void semaphore_init(semaphore_t *sem, int arg)
{
    invoke_syscall(SYSCALL_SEMAPHORE_INIT, (int)sem, (int) arg, IGNORE);
}
...
{
    invoke_syscall(SYSCALL_NETRECV, (uint32_t)rd, (uint32_t)rd_phy, (uint32_t)daddr);
}

void sys_net_send(uint32_t td, uint32_t td_phy)
{
    invoke_syscall(SYSCALL_NETSEND, (uint32_t)td, (uint32_t)td_phy, IGNORE);
}

void sys_init_mac(void)
{
    invoke_syscall(SYSCALL_INITMAC, IGNORE, IGNORE, IGNORE);
}

void sys_wait_recv_package(void)
{
    invoke_syscall(SYSCALL_WAITRECV, IGNORE, IGNORE, IGNORE);
}