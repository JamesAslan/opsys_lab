...
void irq_enable(int IRQn)
{
    ...
    int content = reg_read_32(0xbfd01044 + base*32);
    reg_write_32(0xbfd01044 + base*32,content | change);
    */
   
    reg_write_32(0xbfd01064 ,0xffffffff);
    reg_write_32(0xbfd01068 ,0xffffffff);
    reg_write_32(0xbfd0106c ,0x00000000);
    int content = reg_read_32(0xbfd0105c);
    reg_write_32(0xbfd0105c ,content | 0x00000008);
}
...
void check_recv(mac_t *test_mac)
{
    ...
    for(i=0;i< 64;i++)
    {
        int *content = (int)(dma_buffer_start_rd + 1024*i);
        printf("\t%x\t",*content);
...
uint32_t do_net_recv(uint32_t rd, uint32_t rd_phy, uint32_t daddr)
{
   //PLEASE enable MAC-RX
    reg_write_32(GMAC_BASE_ADDR, reg_read_32(GMAC_BASE_ADDR) | 0x0000000c);
    ...
    /*for(i=0;i< 1024*64;i++)
    {
        my_printk("the data received is "); 
        my_printk("%d ",(int)(dma_buffer_start_rd + 4*i));
    }*/
    ...
    {
        //reg_write_32(DMA_BASE_ADDR + DmaRxPollDemand, 1);
        my_printk("recieving %d",number);
        while((rd_desc[number].tdes0 & 0x80000000) == 0x80000000)
            ;
        my_printk(" recieved ||");
        number++;
    }*/
...
{ 
    ...
    my_printk("send preperating\n");

    int i;
    for(i=0;i<64;i++)
    {
        sd_desc[i].tdes0 = /*sd_desc[i].tdes0 |*/ 0x80000000;
    }

    //you should add some code to start send packages
    
    my_printk("send started\n");
    ...
    {
        reg_write_32(DMA_BASE_ADDR + DmaTxPollDemand, 1);
        my_printk("sending %d",number);
        while((sd_desc[number].tdes0 & 0x80000000) == 0x80000000)
            ;
        //my_printk(" sended ||");
        number++;
    }
    my_printk("64 packets sended\n");
}
