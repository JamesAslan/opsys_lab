...
void TLB_miss(int context)
{
    ...
    if(symbol == 0)
    {
        ...
        PTE_info[current_virtue_ad++].virtue = target + 1024*4;
    ...
    for(i=0;i<PTE_NUMBER;i++)
    {
        if(((PTE_info[i].virtue & 0xffffe000) == target) && (PTE_info[i].asid == asid_input))
        {
            if(PTE_info[i].onsd == 1)
            {
                ...
                page_in_number = i;
                page_in_asid_input = asid_input;
                page_in_index_input = index_input;
                special_spawn2();
                wait(process_id);
                break;
            }
            else if(current_real_ad <MAX_REAL)
            {
                PTE_info[i].real = 0x01080000 + current_real_ad*1024*4;
                //PTE_info[i].usage =1;
                PTE_info[i].valid =1;
                current_real_ad++;
                PTE_info[i+1].real = 0x01080000 + current_real_ad*1024*4;
                //PTE_info[i+1].usage =1;
            ...
            else
            {
                ...
                page_out_number = i;
                page_out_asid_input = asid_input;
                page_out_index_input = index_input;
                special_spawn1();
                wait(process_id);
                break;
            }
...
void page_swap_out(int number,int asid_input,int index_input)
{
    ...
    if(swap_target >= PTE_NUMBER)
        swap_target = 0;
    /*screen_move_cursor(35,screen_cursor_y);
    my_printk("swap out:%d for %d!\n",target,number);
    TLB_load(target,target+1,asid_input,31);*/
    sys_move_cursor(35,screen_cursor_y);
    printf("swap out:%d for %d!\n",target,number);

    int pid = current_running->pid;
    TLB_load(target,target+1,pid,31);

    PTE_info[target].sdaddr = sd_target;
    sdwrite(PTE_info[target].virtue,sd_target,8*1024);
    sd_target += 4*1024;
...
void page_swap_in(int number,int asid_input,int index_input)
{
    ...
    if(swap_target >= PTE_NUMBER)
        swap_target = 0;
    /*screen_move_cursor(35,screen_cursor_y);
    my_printk("swap in:%d to %d!            ",number,target);
    TLB_load(target,target+1,asid_input,31);*/
    sys_move_cursor(35,screen_cursor_y);
    printf("swap in:%d to %d!            ",number,target);
    ...
    PTE_info[number+1].real = PTE_info[target+1].real;
    PTE_info[number+1].valid =1;
    TLB_load(number,number+1,pid,index_input);
    //TLB_load(number,number+1,asid_input,index_input);

    //disable_interrupt();
    sdread(PTE_info[number].virtue,PTE_info[number].sdaddr,8*1024);
    //enable_interrupt();
...
void special_spawn1()
{
    while(pcb[15].status != TASK_EXITED)
        do_scheduler();
    process_id++;
    if(pcb[15].status == TASK_EXITED)
    {
        pcb[15].type = KERNEL_PROCESS;
	    pcb[15].user_context.regs[31] = &page_out;
	    pcb[15].user_context.cp0_epc = &page_out;
        pcb[15].user_stack_top = 0xa0f00000 + 0x15000;
	    pcb[15].status = TASK_READY;
        pcb[15].pid = process_id;
...
void special_spawn2()
{
    ...
    {
        pcb[15].type = KERNEL_PROCESS;
	    pcb[15].user_context.regs[31] = &l;
	    pcb[15].user_context.cp0_epc = &page_in;
        pcb[15].user_stack_top = 0xa0f00000 + 0x15000;
	    pcb[15].status = TASK_READY;
        pcb[15].pid = process_id;
        pcb[15].kernel_context.regs[31] = get_addr();
...
void page_out()
{
    //page_out_number = sis_getpid();
    page_swap_out(page_out_number,page_out_asid_input,page_out_index_input);
    sys_exit();
}
...
{
    //page_out_number = sis_getpid();
    page_swap_in(page_in_number,page_in_asid_input,page_in_index_input);
    sys_exit();
}